...
# my python files
from functions import coder, check_holiday, settings
...
FROM_MAIN,_ = loadUiType(os.path.join(os.path.dirname(__file__),"./data/GUI/jarvis.ui"))
...
class Jarvis(QThread):
    ...
    def takeCommand(self):
        ...
        try:
            ...
            with sr.Microphone() as source:
                ...
                main.jarvisTask(self.printf("Initializing..."),False)  

                r.adjust_for_ambient_noise(source,duration=3)
                r.pause_threshold=2
                r.energy_threshold=250
                r.dynamic_energy_threshold=250
                main.jarvisTask(self.printf("Listening..."),False)  
            ...
            main.jarvisTask(self.printf(f"You said -> {query}\n"),False)
            ...
            # time.sleep(10)
            # main.jarvisTask(self.printf("I sorry, couldn't understand"),False)
            print(e)
            query = None
    ...
    def checkCommand(self,input=None,response=False):
     ...
     # while True:
        ...
        if match(['how are you'],query):

            response = "I am fine. Thanks for asking"
            print(response)
            return [response,'Normal']
        
        elif match(["hello",'hey'], query):
        ...
        elif "i am your friend" in query:
            response = f"Thanks a lot. I am glad to hear that"
            print(response)
            return [response,"Normal"]

        elif 'exit' in query:
            main.jarvisTask(self.printf("Thanks for giving me your time"),False)
            time.sleep(2)
            main.close()
            # exit(app.exec_())
            ...
            response = "If you talk then definately your human."
            print(response)
            return [response,"Normal"]
        ...
        # elif 'open bluestack' in query:
            ...
            try: 
                ...
                date = datetime.datetime.now().strfime("%x %H:%M:%S")
            ...
            if not cam:
                ...
                return ["Photo saved successfully!", 'Success']
            # stop()
        ...
        # elif "send message " in query:
            ...
            time.sleep(1)
            subprocess.call('taskkill /f /im python.exe > nul && python Jarvis2.py',shell=True)
...
class Main(QMainWindow,FROM_MAIN):
    ...
    def __init__(self,parent=None):
        ...
        self.setWindowIcon(QIcon('./data/GUI/ico/jarvis.ico'))
        ...
        self.commandInputBox.setFont(fixedFont)
        self.userCommandArea.setFont(fixedFont)
        self.responseArea.setFont(fixedFont)

        self.movie = QtGui.QMovie("./data/GUI/images/initiating.gif")
        self.loading.setMovie(self.movie)
        self.movie.start()

        self.exitButton.setStyleSheet("background-image:url(./data/GUI/images/exit - Copy.png);\n"
        "border:none;")
        ...
        timer.start(1000)
        speaker = QTimer(self)
        speaker.timeout.connect(self.speaker)
        speaker.start(100)
        self.reply = None
        self.commandButton.clicked.connect(self.userTask)
        self.userCommandArea.setTextBackgroundColor(Qt.gray)

        self.listener = Thread(target=self.microphone,daemon=True)
        self.enableMic.clicked.connect(self.activateMicrophone)
        wisher = Thread(target=self.jarvisTask,args=[startJarvis.wishMe(),False],daemon=True)
        # wisher.start()
        menuActionHandler = QTimer(self)
        menuActionHandler.timeout.connect(self.actions)
        menuActionHandler.start(500)

        command = QTimer(self)
        command.timeout.connect(self.commandButtonFunction)
        command.start(100)

        # self.jarvisTask(startJarvis.wishMe(),False)
        print(time.perf_counter() - startTime)

    def actions(self):

        if self.actionCopy.isChecked():
            if self.commandInputBox.toPlainText() != "":
                self.commandInputBox.copy()
            else:
                self.userCommandArea.copy()
            # self.userCommandArea.clear()
            self.actionCopy.setChecked(False)

        elif self.actionSelect.isChecked():
            self.commandInputBox.selectAll()
            self.actionSelect.setChecked(False)

        elif self.actionClose.isChecked():
            self.close()
        ...
        elif self.actionMinimize.isChecked():
            self.minimumSize()
            self.actionMinimize.setChecked(False)

        elif self.actionReload.isChecked():
            self.jarvisTask('reload')

    # def run(self):
        # self.TaskExection

    def commandButtonFunction(self): 
        if '\n' in self.commandInputBox.toPlainText():
            self.userTask()

    def userTask(self):

        if self.listener.is_alive():
            stopThread(self.listener)
            # # self.listener._stop()
            # self.listener._delete()
            # del self.listener
        res = self.commandInputBox.toPlainText()
        print(res)
        if res.replace(' ','') != '':

            # prev = self.userCommandArea.text()
            # self.userCommandArea.append(f'\t\t{res}')
            self.userCommandArea.append(f'''<font background-color='red'>{startJarvis.getAssData('User name')}:</font> {res}''')
            # self.userCommandArea.append(res)
            # self.response.adjustSize()
            # time.sleep(1)
            self.commandInputBox.clear()
            # self.commandInputBox.setReadOnly(True)
            task = Thread(target=self.jarvisTask,args=[res],daemon=True)
            task.start()
            # self.commandInputBox.setReadOnly(False)
    ...
    def microphone(self):
        ...
        # self.jarvisTask(f'''Listening...''',False)
        # command = self.response.text()

        command = startJarvis.takeCommand()
        if command:
            self.jarvisTask(command)
        else:
            # time.sleep(0.5)
            self.jarvisTask(f'Unable to recognize your\
                 voice try typing the command' ,False)
        # self.isMicrophoneActive = False

    def speaker(self):
        if self.reply:
            startJarvis.speak(self.reply)
            self.reply = False

    def jarvisTask(self,command,run=True,action="Normal"):

        if run:
            task = startJarvis.checkCommand(command)
            self.reply = task[0]
            action = task[1]

        else: self.reply = command

        if action == 'Success': color = 'green'
        elif action == 'Error': color = 'red'
        else: color = 'blue'

        if self.reply:
            self.responseArea.append(f'''<font color='darkblue'>{startJarvis.getAssData('Comman name')}:</font> \
                
                <font color='{color}'>{self.reply}</font>''' )

    def showTime(self):
        self.ts = time.strftime("%A, %d %B, %Y")
        self.dateLable.setText("<font size=5 color='white'>"+self.ts+"</font>")
        ...
        self.timeLable.setText("<font size=5 color='white'>"+label_time+"</font>")
