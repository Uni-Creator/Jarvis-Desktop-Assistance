...
class ChatGUI:
    def __init__(self, callback, first_message="Hello! How may I help you ?"):
        """
        ...
        self.data_path = "./data/GUI/ico/"

        # instance args
        self.callback = callback
        self.terminate1 = "exit"
        self.terminate2 = "quit"
    ...
    def add_bot_message(self, message):
        ...
        # need to update canvas scroll region after content modified
        self.canvas.configure(scrollregion=self.canvas.bbox("all"))
        # speaker = Thread(target=speak,args=(message,),daemon=True)
        # speaker.start()
    ...
    def user_input_handler(self, event):
        ...
        if message == self.terminate1 or message == self.terminate2:
        ...
        self.user_thread = Thread(target=self.add_user_message, args=(message,),daemon=True)
        self.bot_thread = Thread(target=self.process_message, args=(message,),daemon=True)
...
def call(msg):
    return jarvis.generate_response(msg)
    # return 'th tg'
